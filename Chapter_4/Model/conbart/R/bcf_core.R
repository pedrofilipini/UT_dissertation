...
#'}
#' @export
bcf_core <- function(y, tvar_con = NULL, tvar_mod = tvar_mod, z_train = NULL, z_out = z_train,
                            Omega_con=matrix(rep(1,length(y)), ncol=1), Omega_mod=Omega_con,
                            x_control, x_moderate = x_control,
                            tvar_con_out = tvar_con, tvar_mod_out = tvar_con_out,
  ...
  if(vanilla){
    ...
    Phi_con = matrix(rep(1,n), ncol=1) 
    Phi_con_out = matrix(rep(1,nrow(x_control_out)), ncol=1)
    l_con = 1
    mc_con = 1 #except this one, makes the size of the basis dimension
    l_con_aux = 1
    c_con_aux = 1
    ls_prior_con = c(1,1)
    tvar_con = 1
