...
foreach(k = 1:length(hyper), .packages=c('lbart')) %dopar%  {
  ...
  for(i in 1:5){
    ...
    set.seed(1)
    #simtime <- Sys.time()
    lbcf_math1 <- lbart:::bcf_new(y, #response
                                    z = z, z_est = zp, #Z
    ...
    r1_train <- sqrt(mean((apply(lbcf_math1$mu_post+t(t(lbcf_math1$b_post)*z), 2, mean)-y)^2))
    r1 <- sqrt(mean((apply(lbcf_math1$mu_est_post+t(t(lbcf_math1$b_est_post)*zp), 2, mean)-yp)^2))
    ...
    #write.table(time, paste0("times_cv_math__a_", cbase, "__am_",mbase, "__b_",cpower,"__bm_",mpower,"__nt_",ctree,"__ntm_",mtree,"__mu_",mm,"__tau_",tt,"__linear_",linear,"__fold_",i,"_partial.csv"), sep = ";")
    write.table(r1_train, paste0("rmsetrain_cv_math__a_", cbase, "__am_",mbase, "__b_",cpower,"__bm_",mpower,"__nt_",ctree,"__ntm_",mtree,"__mu_",mm,"__tau_",tt,"__linear_",linear,"__fold_",i,"_partial.csv"), sep = ";")
    write.table(r1, paste0("rmse_cv_math__a_", cbase, "__am_",mbase, "__b_",cpower,"__bm_",mpower,"__nt_",ctree,"__ntm_",mtree,"__mu_",mm,"__tau_",tt,"__linear_",linear,"__fold_",i,"_partial.csv"), sep = ";")
