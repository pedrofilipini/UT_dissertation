...

{
  ...
  for(i in 1:length(lista_sz)){
    ...
    #   for(beta in c(1, 2, 3)){
    #     for(nt in c(10,50,100,150)){
          cl <- parallel::makeCluster(20)
          doParallel::registerDoParallel(cl)
          foreach(hh = 1:simrep, .packages=c('lbart','dbarts','dplyr','magrittr','SoftBart')) %dopar%  {
            ...
            soft_default <- softbart(X = x_train, Y = y_train, X_test = x_test,
                                    opts = Opts(num_burn = burn, num_thin = th, num_save = post, update_s = F, update_alpha = F))

            times_soft <- difftime(Sys.time(), simtime, units = "secs")[[1]]
            ...
            r3_train <- sqrt(mean((apply(soft_default$y_hat_train, 2, mean)-y_train)^2))
            ...
            r3 <- sqrt(mean((apply(soft_default$y_hat_test, 2, mean)-y_test)^2))
            ...
            cove3 <- mean(dplyr::between(y_train,apply(soft_default$y_hat_train, 2, quantile, 0.025), apply(soft_default$y_hat_train, 2, quantile, 0.975)))
            ...
            covep3 <- mean(dplyr::between(y_test,apply(soft_default$y_hat_test, 2, quantile, 0.025), apply(soft_default$y_hat_test, 2, quantile, 0.975)))
            ...
            rangep3 <- mean(apply(soft_default$y_hat_test, 2, quantile, 0.975)-apply(soft_default$y_hat_test, 2, quantile, 0.025))
            ...
            range3 <- mean(apply(soft_default$y_hat_train, 2, quantile, 0.975)-apply(soft_default$y_hat_train, 2, quantile, 0.025))
            ...
            write.table(times_vec, paste0("times_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(rmse_train, paste0("rmse_train_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(rmse, paste0("rmse_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(coverage, paste0("coverage_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(coveragep, paste0("coveragep_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(range, paste0("range_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
            write.table(rangep, paste0("rangep_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")
          ...
          for(hh in 1:simrep){
            times_matrix[hh,] <- read.csv(paste0("times_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";") %>% t() %>% as.numeric()
            rmse_matrix_train[hh,] <- read.csv(paste0("rmse_train_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")%>% t() %>% as.numeric()
            rmse_matrix[hh,] <- read.csv(paste0("rmse_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";") %>% t() %>% as.numeric()
            coverage_matrix[hh,] <- read.csv(paste0("coverage_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";") %>% t() %>% as.numeric()
            coveragep_matrix[hh,] <- read.csv(paste0("coveragep_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";") %>% t() %>% as.numeric()
            range_matrix[hh,] <- read.csv(paste0("range_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")%>% t() %>% as.numeric()
            rangep_matrix[hh,] <- read.csv(paste0("rangep_cv_soft__s_off__i_",i,"__hh_",hh,"_partial.csv"), sep = ";")%>% t() %>% as.numeric()
          }
          ...
          write.table(times_matrix, paste0("times_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(rmse_matrix_train, paste0("rmse_train_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(rmse_matrix, paste0("rmse_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(coverage_matrix, paste0("coverage_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(coveragep_matrix, paste0("coveragep_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(range_matrix, paste0("range_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
          write.table(rangep_matrix, paste0("rangep_cv_soft__s_off__i_",i,"_full.csv"), sep = ";")
